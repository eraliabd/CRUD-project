...
def get_teachers():
    with closing(connection.cursor()) as cursor:
        ...
        adminapp_teacher.age, adminapp_kafedra.name as kafedra_name, adminapp_subject.name as subject_name 
        from adminapp_teacher left join adminapp_kafedra on adminapp_teacher.kafedra_id = adminapp_kafedra.id 
...
def get_groups():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT adminapp_group.id, adminapp_group.name, adminapp_faculty.name as faculty_name
...
def get_students():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT adminapp_student.id, adminapp_student.first_name, adminapp_student.last_name, 
         adminapp_student.age, adminapp_group.name as group_name from adminapp_student 
